...
def grad_array(tgrad):
    return [numpy.asarray(g) for g in tgrad]
...
def init_params(options):
    ...
    params = OrderedDict()
    # embedding
    randn = numpy.random.rand(options['feat_dim'],
                              options['dim_proj'])
...
def train_lstm(
    ...
    feat_dim=252,  # CNN feature dimension
    ...
    test_mode=False, # If true, we do test instead
    result_dir=None
):
    ...
    load_data, prepare_data = get_dataset(dataset, data_id)
    ...
    # in test_mode, by pass all model processing
    if test_mode:
        if not result_dir:
            result_dir = 'test_results_eeg'
...
#                                  numpy.array(test[1])[test_index],
#                                  maxlen=None)
        for t in xrange(len(test[0])):
            x, mask, y = prepare_data([test[0][t]], numpy.array(test[1])[t], maxlen=None)
            preds = f_pred_prob_all(x, mask)

            fig = numpy.squeeze(preds[:,0,:])
    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                # # import pdb; pdb.set_trace()
                # print 'gradients :', [numpy.mean(g) for g in grads_value]
                # params = unzip(tparams)
                # print 'parameter :', [numpy.mean(vv) for kk, vv in params.iteritems()]
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    valid_err = 0 #pred_error(f_pred, prepare_data, valid, kf_valid)
                    # test_err = valid_err
                    ...
                    pylab.figure(1); pylab.plot(numpy.array(history_errs))
                    time.sleep(0.1)
...
def train_all():
    model_file = 'lstm_all_model.npz'
    if not os.path.isfile(model_file):
        train_lstm('all', max_epochs=100)

    train_lstm('all', test_mode=True, result_dir='test_result_all')
    
def train_sep(dim_proj=128):
    object_list = ['cup', 'stone', 'sponge', 'spoon', 'knife', 'spatula']
    #object_list = ['sponge']

    for dataset in object_list:
        # training 
        model_file = 'lstm_%s_model.npz' % (dataset)
        if not os.path.isfile(model_file):
            train_lstm(dataset, dim_proj=dim_proj)
        # testing
        train_lstm(dataset, test_mode=True, dim_proj=dim_proj)


if __name__ == '__main__':
    pylab.ion()

    for data_id in xrange(10):
        dataset = 'eeg'
        model_file = 'lstm_%s_%d_model.npz' % (dataset, data_id)
        if True: #not os.path.isfile(model_file):
            train_lstm(dataset, dim_proj=64, data_id=data_id)

        train_lstm(dataset, dim_proj=64, data_id=data_id, test_mode=True)
