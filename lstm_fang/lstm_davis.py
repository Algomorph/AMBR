...
def train_lstm(
    ...
    feat_dim=10800,  # CNN feature dimension
        ...
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    lines = pylab.plot(numpy.array(history_errs))
                    ...
                    if (uidx == 0 or
                        valid_err <= numpy.array(history_errs)[:,1].min()):

                        best_p = unzip(tparams)
                        bad_counter = 0
                        if valid_err < numpy.array(history_errs)[:,1].min():
...
if __name__ == '__main__':
    ...
    for dataset in object_list:
        # training 
        model_file = 'lstm_davis_%s_model.npz' % (dataset)
...
# stdbuf -oL python lstm_davis.py  2>&1 | tee train_davis_ts`date +%s`.log
