...
class App:
    def __init__(self, master):
        ...
        self.ConfigInfo = {"auto_low_light": "Automaticaly increase exposure time in low-light conditions",
                           "auto_power_off": "Power down camera after specified time of inactivity",
                           "burst_capture_number": "Specify ammount of images taken in Burst mode",
                           "buzzer_ring": "Enable/disable camera locator beacon",
                           "buzzer_volume": "Volume of camera beep",
                           "camera_clock": "Tick&Apply to set Camera clock to the same as this PC",
                           "capture_default_mode": "Mode to enter when changing to Capture via system_default_mode/HW button",
                           "capture_mode": "Changes behavior of \"Photo\" button",
                           "emergency_file_backup": "Locks file when shock is detected-for car dashcam (related to \"loop_record\")",
                           "led_mode": "Set preferred LED behavior",
                           "loop_record": "Overwrites oldest files when memory card is full",
                           "meter_mode": "Metering mode for exposure/white ballance",
                           "osd_enable": "Overlay info to hdmi/TV out", "photo_quality": "Set quality of still images",
                           "photo_size": "Set resolution of still images",
                           "photo_stamp": "Overlay date and time of capture to still images",
                           "precise_cont_time": "Delay between individual images in timelapse mode",
                           "precise_selftime": "Set delay to capture in Timer mode",
                           "preview_status": "Turn this on to enable LIVE view",
                           "start_wifi_while_booted": "Enable WiFi on boot",
                           "system_default_mode": "Mode for HW trigger to set when camera is turned on",
                           "system_mode": "Current mode for HW trigger",
                           "timelapse_video": "Create timelapse video from image taken every 2 seconds",
                           "video_output_dev_type": "Select video out HDMI or AV out over USB, use same cable as SJ4000",
                           "video_quality": "Set quality of video recordings",
                           "video_rotate": "Rotate video by 180° (upsidedown mount)",
                           "video_resolution": "video_resolution is limited by selected video_standard",
                           "video_stamp": "Overlay date and time to video recordings",
                           "video_standard": "video_standard limits possible video_resolution options",
                           "warp_enable": "On = No fisheye (Compensation ON), Off = Fisheye (Compensation OFF)",
                           "wifi_ssid": "WiFi network name; reboot camera after Apply to take effect",
                           "wifi_password": "WiFi network password; reboot camera after Apply to take effect"}
        self.ConfigTypes = {"auto_low_light": "checkbutton", "auto_power_off": "optionmenu",
                            "burst_capture_number": "optionmenu", "buzzer_ring": "checkbutton",
                            "buzzer_volume": "optionmenu", "camera_clock": "button",
                            "capture_default_mode": "optionmenu", "emergency_file_backup": "checkbutton",
                            "led_mode": "optionmenu", "loop_record": "checkbutton", "meter_mode": "optionmenu",
                            "osd_enable": "checkbutton", "photo_quality": "optionmenu", "photo_size": "optionmenu",
                            "photo_stamp": "optionmenu", "precise_cont_time": "optionmenu",
                            "precise_selftime": "optionmenu", "preview_status": "checkbutton",
                            "start_wifi_while_booted": "checkbutton", "system_default_mode": "radiobutton",
                            "system_mode": "radiobutton", "timelapse_photo": "radiobutton",
                            "timelapse_video": "radiobutton", "timelapse_video_duration": "entry",
                            "video_output_dev_type": "optionmenu", "video_quality": "optionmenu",
                            "video_resolution": "optionmenu", "video_rotate": "checkbutton",
                            "video_stamp": "optionmenu", "video_standard": "radiobutton", "warp_enable": "checkbutton",
                            "wifi_ssid": "entry", "wifi_password": "entry"}
        self.ConfigIgnores = ["dev_reboot", "restore_factory_settings", "capture_mode", "precise_self_running"]
        self.FileTypes = {"/": "Folder", ".ash": "Script", ".bmp": "Image", ".ico": "Image", ".jpg": "Image",
                          ".mka": "Audio", ".mkv": "Video", "mp3": "Audio", ".mp4": "Video", ".mpg": "Video",
                          ".png": "Image", ".txt": "Text", "wav": "Audio"}
        ...
        if add == "" and rem == "":  # nothing to add or remove = initial call
            try:  # open the settings file (if exists) and read the settings
                ...
                for pname in ConfigFile.keys():
                    ...
                    if pname in (
                    "camaddr", "camauto", "camport", "camdataport", "camwebport", "custom_vlc_path", "DebugMode",
                    "DefaultChunkSize", "ExpertMode", "FileSort"): setattr(self, pname, pvalue)
                if not {"camaddr", "camauto", "camport", "camdataport", "camwebport", "custom_vlc_path"} <= set(
                    ConfigFile.keys()): raise
        ...
        try:
            ...
            while 1:
                if self.connected:
                    if self.token == "":
                        break
                    ToWrite = {"camaddr": self.camaddr, "camauto": self.camauto, "camport": self.camport,
                               "camdataport": self.camdataport, "camwebport": self.camwebport,
                               "custom_vlc_path": self.custom_vlc_path}
    ...
    def ShowExpertMenu(self):
        ...
        self.Expertmenu.add_command(label="Activate camera jetpack", command=self.ActionInfo, state=DISABLED,
                                    underline=0)
    ...
    def ShowExpertChunkMenu(self):
        ...
        for thislabel in sorted(self.ChunkSizes):
            if self.DefaultChunkSize == thislabel * 1024:
                ...
                selected = "  "
            self.ExpertmenuChunkSize.add_command(label="%skB%s" % (thislabel, selected),
                                                 command=functools.partial(self.ExpertChunkChange, thislabel * 1024))
    ...
    def ExpertTelnet(self):
        ...
        self.Comm(tosend)
        if tkMessageBox.askyesno("Restart Camera",
                                 "You have to reboot camera for telnet to be enabled.\n\nReboot now? (C&C will close)"):
    ...
    def MainWindow(self):
        ...
        if self.SDOK:
            ...
            l = Label(self.mainwindow, width=40,
                      text="No SD memory card inserted in camera!\n\nPlease power off camera, insert SD & restart C&C.",
                      anchor=CENTER)
    ...
    def ExpertEnable(self):
        if tkMessageBox.askyesno("Enable expert mode",
    ...
    def ActionInfo(self):
        ...
        self.camconfig["sw_version"], self.camconfig["hw_version"], self.camconfig["serial_number"]))
        self.UpdateUsage()

    def ActionForceFormat(self):
        if tkMessageBox.askyesno("Format memory card",
    ...
    def ActionFactory(self):
        if tkMessageBox.askyesno("Reboot camera",
    ...
    def MenuConfig_Commit(self, ThisOption, ThisValue):
        ...
        tosend = '{"msg_id":2,"token":%s, "type":"%s", "param":"%s"}' % (
        self.token, ThisOption, ThisValue.replace("/", "\\/"))
        self.ReadConfig()
        self.Comm(tosend)
        ...
        for ThisOption in self.config_options:
            ...
            if ThisOption == "video_resolution":
                ...
                if ThisType == "optionmenu":
                    ...
                    ValueBox = Checkbutton(ThisFrame, text=self.camconfig[ThisOption], variable=ThisValue, onvalue="on",
                                           offvalue="off")
                    ValueBox.config(width=18, anchor=W)
                    ValueBox.grid(row=row, column=1)
                    self.status.append((ValueBox, ThisValue))
                    ...
                    ValueBox1 = Radiobutton(RadioFrame, text=self.camsettableconfig[ThisOption][0], variable=ThisValue,
                                            value=self.camsettableconfig[ThisOption][0])
                    ValueBox1.config(width=7, anchor=W)
                    ValueBox1.pack(side=LEFT)
                    ValueBox2 = Radiobutton(RadioFrame, text=self.camsettableconfig[ThisOption][1], variable=ThisValue,
                                            value=self.camsettableconfig[ThisOption][1])
            ...
            if ThisOption in self.ConfigInfo:
                ThisHint = Label(ThisFrame, width=62, text="*%s" % self.ConfigInfo[ThisOption], anchor=W).grid(row=row,
                                                                                                               column=2,
                                                                                                               padx=10)
            else:
                ThisHint = Label(ThisFrame, width=62,
                                 text='* Unknown config option, if you know what it does, let me know.', anchor=W).grid(
    ...
    def FileDownReport(self, bytes_so_far, chunk_size, total_size, FileTP):
        ...
        if (thistime - self.FileTime > 0.01) and self.connected:
            ...
            self.FileProgress.config(text="Downloading %s at %s/s (%0.2f%%, %ss left)" % (
            FileTP, self.GetPres(Value=AvgSpeed), percent, remaining), bg=self.defaultbg)
    ...
    def FileDownChunk(self, chunk_size=0, report_hook=None, FileTP=""):
        ...
        if thisPwd.startswith("/var/www/DCIM") or thisPwd.startswith("/tmp/fuse_d/DCIM"):
            ...
            tosend = '{"msg_id":1285,"token":%s,"param":"%s", "offset":0, "fetch_size":%s}' % (
            self.token, FileTP, self.FileSize[FileTP])
            resp = self.Comm(tosend)
    ...
    def FileUpReport(self, bytes_so_far, chunk_size, total_size, FileTP):
        ...
        if (thistime - self.FileTime > 0.01) and self.connected:
            ...
            self.FileProgress.config(text="Uploading %s at %s/s (%0.2f%%, %ss left)" % (
            FileTP, self.GetPres(Value=AvgSpeed), percent, remaining), bg=self.defaultbg)
    ...
    def FileUpChunk(self, chunk_size=0, report_hook=None):
        ...
        tosend = '{"msg_id":1286,"token":%s,"md5sum":"%s", "param":"%s", "offset":%s, "size":%s}' % (
        self.token, ThisMD5, ThisFileName, ThisPosition, total_size)
        resp = self.Comm(tosend)
        if int(resp["rval"]) == 0:
            while self.connected:
                ThisSize = chunk_size
    ...
    def FileDelete(self, *args):
        try:
            ...
            if tkMessageBox.askyesno("Delete file",
                                     "Are you sure you want to DELETE\n\n%s\n\nThis action can't be undone!" % FilesToProcessStr):
                if self.ExpertMode == "":
                    tosend = '{"msg_id":1283,"token":%s,"param":"\/var\/www\/DCIM\/%s"}' % (
                    self.token, self.MediaDir)  # make sure we are still in the correct path
                    self.Comm(tosend)
                for FileTP in FilesToProcess:
    ...
    def FileCreateListLabel(self):
        if self.FileSort == 0:
            self.LabelFileType = Button(self.LabelFileHead, width=14, height=1, text="Type ▲", anchor=S,
                                        command=self.FileChangeOrderType)
        ...
        if self.FileSort == 2:
            self.LabelFileName = Button(self.LabelFileHead, width=35, height=1, text="Filename ▲", anchor=S,
                                        command=self.FileChangeOrderName)
        ...
        if self.FileSort == 4:
            self.LabelFileSize = Button(self.LabelFileHead, width=12, height=1, text="Size ▲", anchor=S,
                                        command=self.FileChangeOrderSize)
        ...
        if self.FileSort == 6:
            self.LabelFileDate = Button(self.LabelFileHead, width=30, height=1, text="DateTime ▲", anchor=S,
                                        command=self.FileChangeOrderDate)
    ...
    def FileCreateList(self):
        ...
        self.ListboxFileType = Listbox(self.filelist, height=8, width=12, bd=0, bg="#ffffff", fg="#000000",
                                       activestyle=NONE, highlightcolor="#ffffff", highlightthickness=0,
                                       selectborderwidth=0, selectbackground="#ffffff", selectforeground="#000000")
        self.ListboxFileName = Listbox(self.filelist, yscrollcommand=self.FileYScrollKey, selectmode=EXTENDED,
                                       height=27, width=37, bd=0, bg="#ffffff", fg="#000000", highlightcolor="#ffffff",
                                       highlightthickness=0)
        self.ListboxFileName.bind("<Double-Button-1>", self.FileDoubleClick)
        self.ListboxFileName.bind("<Return>", self.FileDoubleClick)
        self.ListboxFileName.bind("<BackSpace>", self.KeyCwd)
        self.ListboxFileName.focus_set()
        self.ListboxFileSize = Listbox(self.filelist, height=8, width=12, bd=0, bg="#ffffff", fg="#000000",
                                       activestyle=NONE, highlightcolor="#ffffff", highlightthickness=0,
                                       selectborderwidth=0, selectbackground="#ffffff", selectforeground="#000000")
        self.ListboxFileDate = Listbox(self.filelist, height=8, width=30, bd=0, bg="#ffffff", fg="#000000",
                                       activestyle=NONE, highlightcolor="#ffffff", highlightthickness=0,
                                       selectborderwidth=0, selectbackground="#ffffff", selectforeground="#000000")
    ...
    def FilePrintList(self):
        ...
        if len(FileListing) == 0 and self.ExpertMode == "":
            ...
            if self.FileSort == 0:
                FileSorted = sorted(CamFiles, key=itemgetter(0))
            elif self.FileSort == 1:
                FileSorted = sorted(CamFiles, key=itemgetter(0), reverse=True)
            elif self.FileSort == 2:
                FileSorted = sorted(CamFiles, key=itemgetter(1))
            elif self.FileSort == 3:
                FileSorted = sorted(CamFiles, key=itemgetter(1), reverse=True)
            elif self.FileSort == 4:
                FileSorted = sorted(CamFiles, key=itemgetter(2))
            elif self.FileSort == 5:
                FileSorted = sorted(CamFiles, key=itemgetter(2), reverse=True)
            elif self.FileSort == 6:
                FileSorted = sorted(CamFiles, key=itemgetter(3))
            elif self.FileSort == 7:
                FileSorted = sorted(CamFiles, key=itemgetter(3), reverse=True)
    ...
    def FileManager(self, *args):
        ...
        if self.MediaDir != "":
            tosend = '{"msg_id":1283,"token":%s,"param":"\/var\/www\/DCIM\/%s"}' % (
            self.token, self.MediaDir)  # lets seth final path in camera
            self.curPwd = self.Comm(tosend)["pwd"]
        if self.ExpertMode != "":
            self.curPwdFrame = Frame(self.topbuttons)
            self.curPwdLabel = Label(self.curPwdFrame, width=30, bd=1, relief=RIDGE, bg="#aaaaff",
                                     text=self.curPwd.replace('\/', '/')[-35:], anchor=W)
        ...
        if self.ExpertMode != "":
            ...
            self.FileButtonCwd = Button(self.content, text="Change folder", width=11, bg="#ffff66",
                                        command=self.FileCwd)
        ...
        self.FileProgress = Label(self.content, width=60, anchor=W, bd=1, relief=SUNKEN, text="Select a file",
                                  bg=self.defaultbg)
