...
class App:
    ...
    def comm(self, tosend):
        ...
        for cam_ix in self.CamSockets.keys():
            Jtosend["token"] = self.CamTokens[cam_ix]
            tosend = json.dumps(Jtosend)
            self.JsonData[cam_ix][msgid] = ""
            self.CamSockets[cam_ix].send(tosend)

        for cam_ix in self.CamSockets.keys():
            while self.JsonData[cam_ix][msgid] == "": continue
            if self.JsonData[cam_ix][msgid][
                ...
                while self.CamTokens[cam_ix] == "": continue
                Jtosend["token"] = self.CamTokens[cam_ix]
                tosend = json.dumps(Jtosend)
                self.JsonData[cam_ix][msgid] = ""
                self.CamSockets[cam_ix].send(tosend)
    ...
    def build_menu_controls(self, *args):
        ...
        if self.camconfig[0]["capture_mode"] == "precise quality":
            ...
            if self.camconfig[0]["precise_cont_capturing"] == "off":
                self.bphoto = Button(self.controlbuttons, text="Start\nTIMELAPSE", width=13, command=self.action_take_photo,
                                     bg="#66ff66")
            else:
        ...
        self.brecord = Button(self.controlbuttons, text="Start\nRecording", width=7, command=self.action_start_recording,
                              bg="#66ff66")
