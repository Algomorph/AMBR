...
def compute_prediction_error(classify_sequence, data, index_sets, compute_histogram_function=None):
    """
...
def test_lstm(model_output_path, args, result_dir=None):
    ...
    test_minibatch_index_sets = get_batch_indices(len(test_data), args.validation_batch_size)
    print("%d test examples" % len(test_data))

    class_probabilities, true_labels, precision, recall = compute_prediction_precision_and_recall(
        compute_sequence_class_probabilities,
        test_data,
        test_minibatch_index_sets,
        n_categories, verbose=False)
    predicted_labels = np.argmax(class_probabilities, axis=1)
    cm = calculate_confusion_matrix(true_labels, predicted_labels, category_count=n_categories)
...
def train_lstm(model_output_path, args, check_gradients=False):
    ...
    print('Loading data...')
    print('Sample counts:')
    training_dataset, validation_dataset, test_dataset, n_categories, n_features = load_data(args.datasets, args.folder)
    ...
    gradients = tensor.grad(compute_loss, wrt=list(parameters.values()))
    compute_gradients = theano.function(inputs, gradients, name='f_grad')

    lr = tensor.scalar(name='lr')
    f_grad_shared, update_parameters = build_optimizer(lr, parameters, gradients,
                                                       batch_features, mask, batch_labels, compute_loss, w)

    print('Training the model...')

    validation_batch_indices = get_batch_indices(len(validation_dataset), args.validation_batch_size)
    test_batch_indices = get_batch_indices(len(test_dataset), args.validation_batch_size)
    ...
    try:
        for epoch_index in range(args.max_epochs):
            ...
            train_minibatch_indices = get_batch_indices(len(training_dataset), args.batch_size, shuffle=True)
            ...
            for training_minibatch_indices in train_minibatch_indices:
                ...
                if check_gradients:
                    ...
                    print('gradients :', [np.mean(g) for g in theano_to_numpy_grad_array(gradients)])
                    print('parameters :', [np.mean(vv) for kk, vv in parameters.as_dict().items()])

                loss = f_grad_shared(*inputs)
                update_parameters(args.learning_rate)
                ...
                if (current_update_index + 1) % args.display_interval == 0:
                    print('Epoch ', epoch_index, 'Update ', current_update_index, 'Cost ', loss)
                ...
                if (current_update_index + 1) % args.validation_interval == 0:
                    ...
                    test_error = compute_prediction_error(classify_sequence, test_dataset, test_batch_indices)
                    ...
                    if current_update_index == 0 or validation_error <= np.array(error_history)[:, 1].min():
                        best_parameters = parameters.as_dict()  # save best validation results
                        bad_counter = 0
    ...
    use_noise_flag.set_value(0.)
    sorted_train_minibatch_index_sets = get_batch_indices(len(training_dataset), args.batch_size)
    training_error = compute_prediction_error(classify_sequence, training_dataset, sorted_train_minibatch_index_sets)
    validation_error = compute_prediction_error(classify_sequence, validation_dataset, validation_batch_indices)
    test_error = compute_prediction_error(classify_sequence, test_dataset, test_batch_indices)
